...
def get_top_n_predictions(patient_symptoms, n=4):
    ...
    top_indices = probs.argsort()[-n:][::-1]  # Top N indices
    return [(diseases_list[idx], probs[idx]) for idx in top_indices]
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms" or not symptoms.strip():
            message = "Please enter symptoms correctly"
        ...
        if not user_symptoms:
            message = "No valid symptoms detected"
            return render_template('index.html', message=message)

        predicted_diseases = get_top_n_predictions(user_symptoms)

        if not predicted_diseases:
            message = "No diseases matched the symptoms."
            return render_template('index.html', message=message)

        # Collect helper info for each predicted disease
        descriptions = {}
        precautions_dict = {}
        medications_dict = {}
        diets_dict = {}
        ...
        for disease, prob in predicted_diseases:
            desc, pre, meds, diet, wrk = helper(disease)
            descriptions[disease] = desc
            precautions_dict[disease] = pre
            medications_dict[disease] = meds
            diets_dict[disease] = diet
            workouts_dict[disease] = wrk

        return render_template('index.html',
                               predicted_diseases=predicted_diseases,
                               entered_symptom=symptoms,
                               descriptions=descriptions,
                               precautions=precautions_dict,
